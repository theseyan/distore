...
const {formatSize, fetchAll} = require('./util');
const stream = require('node:stream');

// Max chunk size in bytes
const MAX_CHUNK_SIZE = 24 * 1024 * 1024; // 24 MiB

// Max byte range size for fetching
const MAX_BYTE_RANGE = 128 * 1024 * 1024; // 128 MiB
    ...
    fileManager = {
        ...
        */
        uploadChunk: async (id, filepath, index) => {
            ...
            let end = start + MAX_CHUNK_SIZE - 1;

            // Read entire range of file
            let chunks = [];
            for await (let chunk of fs.createReadStream(filepath, { start, end, autoClose: true })) {
                chunks.push(chunk);
            }
            chunks = Buffer.concat(chunks);

            // Make sure range end is correct
            if(chunks.length < MAX_CHUNK_SIZE) end = start + chunks.length - 1;
        ...
        */
        uploadFile: async (filepath, dbpath, parallel, updateFn) => {
            ...
            // Register directories in database
            await this.db.registerPath(path.dirname(dbpath));
        ...
        /**
         * Returns given byte range from File of given ID as a stream.
         * @param {string} id - File ID
         * @param {number} start - Starting byte offset
         * @param {number} end - Ending byte offset
        */
        getBytes: async (id, start, end) => {
            let readable = new stream.Readable();
            readable._read = () => {};
            
            // Fetch list of chunks for range
            const chunksMeta = await this.db.getChunksFromByteRange(id, start, end);

            // Validate byte range
            if(start < 0 || chunksMeta.length<1 || end > chunksMeta[chunksMeta.length - 1].range.end) throw new Error('INVALID_RANGE');
            ...
            let startFetch = async () => {
                for(let i=0; i<chunksMeta.length; i++) {
                    const data = await downloadChunk(i);
                    const chunk = chunksMeta[i];

                    if(start > chunk.range.start && end < chunk.range.end) {
                        readable.push(data.data.subarray(start - chunk.range.start, end - chunk.range.start));
                    }
                    else if(start > chunk.range.start && end >= chunk.range.end) {
                        readable.push(data.data.subarray(start - chunk.range.start));
                    }
                    else if(chunk.range.start >= start && chunk.range.end <= end) {
                        readable.push(data.data);
                    }
                    else if(chunk.range.start >= start && chunk.range.end > end) {
                        readable.push(data.data.subarray(0, end - chunk.range.start));
                    }
                }
                readable.push(null);
            };
    ...
    db = {
        ...
        */
        searchFile: async (name) => {
            ...
            const result = await files.fetch({
                "search?contains": name.toLowerCase()
            });
        ...
        /**
         * Fetches a list of files for a given path in virtual filesystem.
         * @param {string} dirpath - Path to directory in virtual filesystem
        */
        listDirectory: async (dirpath) => {
            const files = this.db.deta.Base('files');
            const directories = this.db.deta.Base('directories');

            // Remove trailing slash
            if(dirpath.endsWith('/') && dirpath != '/') dirpath = dirpath.substring(0, dirpath.length - 1);

            const listFiles = await fetchAll(files, {
                path: dirpath
            });
            const listDirs = await fetchAll(directories, {
                parent: dirpath
            });
        ...
        /**
         * Recursively creates directories in the database to construct the given full path.
         * @param {string} fullpath - Full path to most nested directory
        */
        registerPath: async (fullpath) => {
            const directories = this.db.deta.Base('directories');

            if(fullpath === '/') return;
            else {
                // Check if path already exists
                if(await directories.get(fullpath) != null) return;
            ...
            for(let i=0; i<dirs.length; i++) {
                ...
                await directories.put({
                    parent: parent
                }, current);

                parent += (parent === `/` ? dirs[i] : `/` + dirs[i]);
            }
        },
        ...
        */
        addFile: async (name, dirpath, size) => {
            ...
            return await files.put({
                ...
                search: name.toLowerCase() // Required for searching
            }, id);
        },
        ...
        */
        getChunks: async (id) => {
            ...
            let chunksList = await fetchAll(chunks, {
                file: id
            });

            // Sort chunks by index in ascending order
        ...
        /**
         * Fetches the list of chunks for a given File ID, at given byte range
         * @param {string} id - File ID
         * @param {number} start - Starting byte offset
         * @param {number} end - Ending byte offset
        */
        getChunksFromByteRange: async (id, start, end) => {
            const chunks = this.db.deta.Base('file_chunks');

            let chunksList = await fetchAll(chunks, [
                // Chunks with start >= range start && end <= range end
                {
                    "file": id,
                    "range.start?gte": start,
                    "range.end?lte": end
                },
                // Chunks with start <= range end && end >= range end
                {
                    "file": id,
                    "range.start?lte": end,
                    "range.end?gte": end
