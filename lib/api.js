    ...
    fileManager = {
        ...
        */
        downloadFile: async (id, savePath) => {
            ...
            console.log(chalk.blueBright("download"), `${file.name} (${file.size} bytes, ${chunksMeta.length} chunks)`, `→`, chalk.yellowBright(savePath), chalk.gray(`[${id}]`));
            ...
            this.console.success(`Saved to \`${savePath}\``);
        },
        ...
        */
        uploadFile: async (filepath, dbpath) => {
            let stat;
            try {
                stat = await fs.promises.stat(filepath);
            }catch(e) {
                if(e.code === 'ENOENT') return this.console.error(`No such file or directory`);
                else return this.console.error(`Failed to stat file \`${filepath}\``);
            }
            ...
            console.log(chalk.blueBright("upload"), path.basename(filepath), `(${stat.size} bytes, ${chunks} chunks)`, `→`, chalk.yellowBright(dbpath), chalk.gray(`[${fileResult.key}]`));
            ...
            this.console.success("File uploaded");
